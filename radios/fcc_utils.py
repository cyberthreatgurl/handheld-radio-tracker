...
from radios.models import Radio, RadioFCCTestReport
...
DEFAULT_RADIO_ALLOWLIST_TERMS = "TRANSCEIVER,TRANSMITTER,RECEIVER,MURS"
...
def fetch_fcc_secondary_metadata(fcc_id):
    ...
    if not grantee_code or not product_code:
        return {'record_count': 0, 'text_blob': '', 'matched_keys': [], 'test_report_candidates': []}
...
def fetch_and_sync_fcc_id(fcc_id_query):
    ...
    logger.info(
        "FCC sync completed query=%s added=%s updated=%s exact_grantee=%s skipped_non_exact=%s skipped_non_radio=%s skipped_stale_lookup=%s attached_test_reports=%s",
        fcc_id_query,
        count_added,
        count_updated,
        exact_grantee,
